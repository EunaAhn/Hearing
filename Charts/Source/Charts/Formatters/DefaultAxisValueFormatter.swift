...
@objc(ChartReverseAxisValueFormatter)
open class ReverseAxisValueFormatter: NSObject, AxisValueFormatter
{
    public typealias Block = (
        _ value: Double,
        _ axis: AxisBase?) -> String
    ...
    @objc public static func with(block: @escaping Block) -> ReverseAxisValueFormatter?
    {
        return ReverseAxisValueFormatter(block: block)
    }
                               ...
                               axis: AxisBase?) -> String
    {
        if let block = block {
            ...
            return "      \(formatter?.string(from: NSNumber(floatLiteral: 100 - value)) ?? "")"
        }
    }
}
